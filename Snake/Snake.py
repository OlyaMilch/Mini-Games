...
# Constants
width_game = 600  # Screen width
height_game = 600  # Screen height
body_parts = 3  # Starting blocks of a snake
font = ('Consolas', 20)  # Label font settings

# Variables - some vars are declared later
speed = 110  # Default speed - update once each 100ms
snake_color = '#0099ff'  # Default snake color
food_color = '#d61cbd'  # Default food color
...
class Food():
    def __init__(
            self):  # This should be exclusive (food should spawn in the field), so -1,. Sadly this sometimes will spawn the food above the snake
        ...
        self.coordinates = [x_coords,
                            y_coords]  # Create a new food object                                                              # To make deletion easier
...
def next_turn(snake, food):
    ...
    if check_collisions(snake) == False:
        game_over()
    else:
...
def check_collisions(snake):
    ...
    if x < 0 or y < 0 or x >= width_game or y >= height_game:
        return False
...
score_label = Label(window, font=font, text='Start the game or customize the settings!')
score_label.pack()
